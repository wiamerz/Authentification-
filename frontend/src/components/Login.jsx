...
//   const navigate = useNavigate();
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      //   // Store username && role in localStorage
      //   localStorage.setItem("username", savedData.username);
      //   localStorage.setItem("userRole", savedData.role);
  ...
  return (
    <div
      className="min-h-screen w-full bg-cover bg-center bg-no-repeat flex flex-col justify-center items-center  bg-[rgb(108,88,76)]  px-4" >
      ...
      <div className="bg-[rgb(246,233,215)] p-8 md:p-12 rounded-lg shadow-2xl max-w-md w-full backdrop-blur-sm">
        ...
        <p className="text-center text-black mt-4">
          Don't have an account? <a href="/signup" className="text-[rgb(161,193,129)]">Sign up</a>
        </p>
      </div>
    </div>
  );
